...
% 如果.mat文件中包含采样率信息，可以这样获取：
% if isfield(data_struct_48k, 'fs')
%     fs_48k = data_struct_48k.fs
% end
% if isfield(data_struct_12k, 'fs')
%     fs_12k = data_struct_12k.fs
% end

% 检查采样率是否正确
if fs_48k ~= 48000
    warning('48kHz数据的采样率不是48kHz，实际为: %d Hz', fs_48k)
end

if fs_12k ~= 12000
    warning('12kHz数据的采样率不是12kHz，实际为: %d Hz', fs_12k)
end

% 如果数据是多通道的，只取第一通道
if size(data_48k, 2) > 1
    data_48k = data_48k(:, 1)
    disp('48kHz数据是多通道的，只使用第一通道')
end

if size(data_12k, 2) > 1
    data_12k = data_12k(:, 1)
    disp('12kHz数据是多通道的，只使用第一通道')
end
...
% 确保数据长度匹配（以降采样后的数据为准）
target_fs = 12000
decimation_factor = fs_48k / target_fs

% 计算降采样后的预期长度
expected_length = ceil(length(data_48k) / decimation_factor)

% 如果12kHz数据更长，截断它
if length(data_12k) > expected_length
    data_12k = data_12k(1:expected_length)
    fprintf('截断12kHz数据以匹配预期长度: %d样本\n', expected_length)
end

% 如果12kHz数据更短，用零填充（可选）
if length(data_12k) < expected_length
    padding = zeros(expected_length - length(data_12k), 1)
    data_12k = [data_12k padding]
    fprintf('用零填充12kHz数据至长度: %d样本\n', expected_length)
end
...
fprintf('比较数据长度: %d样本 (%.2f秒)\n', min_length, min_length/target_fs)
...
fprintf('\n=== 误差指标比较 ===\n')
fprintf('均方误差 (MSE):\n')
fprintf('  decimate方法: %.6f\n', mse_decimate)
fprintf('  手动方法:     %.6f\n', mse_manual)

fprintf('\n信噪比 (SNR):\n')
fprintf('  decimate方法: %.2f dB\n', snr_decimate)
fprintf('  手动方法:     %.2f dB\n', snr_manual)

fprintf('\n相关系数:\n')
fprintf('  decimate方法: %.6f\n', corr_coef_decimate(1,2))
fprintf('  手动方法:     %.6f\n', corr_coef_manual(1,2))

fprintf('\n最大绝对误差:\n')
fprintf('  decimate方法: %.6f\n', max_abs_error_decimate)
fprintf('  手动方法:     %.6f\n', max_abs_error_manual)
...
%% 保存比较结果（可选）
% 将降采样后的数据保存为.mat文件，以便进一步分析
% save('downsampled_48k_to_12k.mat', 'data_48k_downsampled', 'target_fs')

% 保存比较结果到MAT文件
% save('comparison_results.mat', 'data_12k', 'data_48k_downsampled', ...
%      'mse_decimate', 'snr_decimate', 'corr_coef_decimate', ...
%      'Pxx_12k', 'f_12k', 'Pxx_ds', 'f_ds')
